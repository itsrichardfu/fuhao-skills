...
| Skill | 它能帮你完成什么 | 状态 |
|---|---|---|
| [`fuhao-learning-loop`](skills/fuhao-learning-loop/) | 把材料、主题或现实问题，推进为能够独立回忆、审辩、应用、迁移并复测的能力 | v0.7.1 |
...
- 把讲解深度和答题难度分开控制；每轮只推进一个认知动作，多字段验证会拆成连续小轮；
- 觉得负担大时可以说“简单一点”“分步来”“给个例子”或“先讲清楚”，系统会降一档并只留下一个小问题；连续两轮稳定复现后才会增加挑战；
...
仓库不包含账号凭据或学习记录。版本检查只读取 GitHub 上公开的 `SKILL.md` 版本号，并在本机缓存检查时间；不会上传学习材料。宿主无法运行脚本或无法联网时会静默跳过检查，不影响学习。发现新版时，Skill 会在首次有效学习结果的同一条用户可见回复末尾显示当前版本、最新版本和更新地址；版本相同或检查失败时不显示。

### 更新

Skill 每个新会话最多检查一次公开版本，24 小时内使用缓存。发现新版时只提醒，不会自行覆盖。提示必须出现在首次有效学习结果的最终回复中，不能只出现在思考或进度消息里。看到提醒后，把上面的同一条安装口令再次发给 Agent，它会先备份旧版，再验证更新与回退路径。
